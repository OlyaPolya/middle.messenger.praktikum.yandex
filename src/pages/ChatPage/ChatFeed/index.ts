...
import { chatFeed } from "../../../pages/ChatPage/ChatFeed/ChatFeedTemplate";
  ...
  const chatList = userMessages.messages.reduce((concat, message) => {
    ...
    const messageType =
      message.message.text.length > 0
        ...
        : `<img src="${message.message.media}" alt="image from user">`;
    const isUserRecipientOrSender =
      message.sender.id === CURRENT_USER_TEMP_ID ? "recipient" : "sender";
    const chowCheckboxReading =
      message.sender.id === CURRENT_USER_TEMP_ID
